...
import com.gaarx.tvplayer.data.services.http.HttpClient
...
class ApiService (
    ...
    fun getURLFromChannelSource(streamSource: StreamSourceItem, proxy: Proxy? = null): String? {
        val finalUrl: String? = when (streamSource.streamSourceType) {
            ...
            StreamSourceTypeItem.YOUTUBE -> {
                processYoutubeSource(streamSource)
            }
    ...
    fun processJSONApiCall(apiCall: ApiCallItem, variableStore: MutableMap<String, String>, proxy: Proxy? = null){
        ...
        apiResponseKeys.forEach { apiResponseKey ->
            ...
            if (storeKey.isNotEmpty() && !extractedValue.isNullOrEmpty()) {
    ...
    fun processTwitchSource(streamSource: StreamSourceItem, proxy: Proxy? = null) : String {
        ...
        val accessTokenValue =
            JsonPath.parse(data)?.read<String>("[3].data.streamPlaybackAccessToken.value")
        val accessTokenSignature =
            JsonPath.parse(data)?.read<String>("[3].data.streamPlaybackAccessToken.signature")

        finalUrl =
            "https://usher.ttvnw.net/api/channel/hls/${id}.m3u8?acmb=e30=&allow_source=true&fast_bread=true&p=&play_session_id=&player_backend=mediaplayer&playlist_include_framerate=true&reassignments_supported=true&sig=${accessTokenSignature}&supported_codecs=avc1&token=${accessTokenValue}&transcode_mode=vbr_v1&cdm=wv&player_version=1.20.0"
        return finalUrl
    }

    fun processYoutubeSource(streamSource: StreamSourceItem) : String? {
        var finalUrl: String?
        try {
